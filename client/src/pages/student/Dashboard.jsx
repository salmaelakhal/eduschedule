...
export default function StudentDashboard() {
  ...
  const getColor   = (subjectId) => EVENT_COLORS[(subjectId - 1) % EVENT_COLORS.length];

  const className = schedules[0]?.class?.name || 'Étudiant';  // ← depuis l'API, pas user

  const STATS = [
    { label: 'Séances / semaine', value: schedules.length, icon: BookOpen,      color: 'var(--color-accent)'  },
    { label: 'Ma classe',         value: className,        icon: GraduationCap, color: 'var(--color-accent3)' },
  ];
    ...
    <div className="content-area">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, marginBottom: 24, maxWidth: 500 }}>
        {STATS.map((stat, i) => (
          <div key={i} className="stat-card">
            ...
            <div className="stat-value" style={{ fontSize: typeof stat.value === 'string' ? 14 : 28 }}>{stat.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-text2)' }}>
          <span style={{ color: 'var(--color-accent3)' }}>📅</span>{' '}Mon emploi du temps —{' '}
              ...
              {uniqueHours.map((h) => {
                ...
                return (
                  <div key={`${day}-${h.startTime}`} className="sg-cell" style={{ cursor: 'default' }}>
                    {seance && (
                      <div className="sg-event" style={{ background: color.bg, borderLeft: `3px solid ${color.border}` }}>
                        <div className="sg-event-subject" style={{ color: color.text }}>{seance.subject?.name}</div>
                        <div className="sg-event-info">👩‍🏫 {seance.teacher?.fullName?.split(' ')[1]}</div>
      ...
      <div style={{ marginTop: 12, fontSize: 11, color: 'var(--color-text2)', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        {EVENT_COLORS.map((c, i) => (
          <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: 2, background: c.bg, border: `2px solid ${c.border}` }} />
            Matière {i + 1}
          </span>
        ))}
